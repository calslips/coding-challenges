...
function goodVsEvil(good, evil){
  ...
  for (let i = 0; i < good.length; i++) {
    goodPoints += good[i] * goodWorth[i];
  }
  for (let j = 0; j < evil.length; j++) {
    evilPoints += evil[j] * evilWorth[j];
  }
  return `Battle Result: ${goodPoints > evilPoints ? 'Good triumphs over Evil' : goodPoints < evilPoints ? 'Evil eradicates all trace of Good' : 'No victor on this battle field'}`;
}
