...
Elixir: @morse_codes variable (from use MorseCode.Constants).
  Ignore the unused variable warning for morse_codes because it's
  no longer used and kept only for old solutions.
...
NASM: a table of pointers to the morsecodes,
  and a corresponding list of ascii symbols
...
Parameters: a string, in morse code
Returns: a string, translated from morse code
Examples:
  decodeMorse('.... . -.--   .--- ..- -.. .') // 'HEY JUDE'
Pseudocode:
  initialize a result variable assigned to an empty string
  split morse string on spaces
    access morse dictionary at current key
      concat value (or space) with result str
  return result str with extra spaces removed
*/

decodeMorse = function(morseCode) {
  return morseCode.split(' ').reduce((s, c) => s += MORSE_CODE[c] || ' ', '').split('  ').join(' ').trim();
}