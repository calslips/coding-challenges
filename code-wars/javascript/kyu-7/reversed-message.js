 ...
 * lower case and reversed. In addition, capitalise the first letter of the
 * newly reversed words. If a number or symbol(!#,>) is now in the first
...
function reverseMessage(str) {
  return str.split(' ').reverse().map(w => [...w].reverse().map((l, i) => i ? l.toLowerCase() : l.toUpperCase()).join('')).join(' ');
}