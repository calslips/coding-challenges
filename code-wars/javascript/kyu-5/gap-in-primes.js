...
2 to 3 the gap is 1. From 3 to 5 the gap is 2. From 7 to 11 it
is 4. Between 2 and 50 we have the following pairs of 2-gaps
...
exist otherwise `nil or null or None or Nothing (or ... depending
  on the language).
...
gap(6,100,110) --> nil or {0, 0} or ... : between 100 and 110 we
have 101, 103, 107, 109 but 101-107is not a 6-gap because there
is 103in between and 103-109is not a 6-gap because there is 107
in between.
...
Returns:
  an arr of 2 nums
    first primes between range with gap length
  null
    if no such values exist within range
Examples:
...
Pseudocode:
  initialize prime var assigned to null
  iterate from start through end
    if current counter is prime
      if prime var has a value and counter minus prime equals gap
        return an arr containinng prime and current counter
      otherwise
        reassign prime to equal current counter
  return null
*/
