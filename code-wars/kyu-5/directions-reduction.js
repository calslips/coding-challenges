...
Parameters: an array of strings representing cardinal directions
Return: an array of directions with opposite adjacent directions removed
Examples:
  dirReduc(["NORTH", "SOUTH", "SOUTH", "EAST", "WEST", "NORTH", "WEST"]) // ["WEST"]
  dirReduc(["NORTH", "WEST", "SOUTH", "EAST"]) // ["NORTH", "WEST", "SOUTH", "EAST"]
  dirReduc(["NORTH", "SOUTH", "EAST", "WEST", "EAST", "WEST"]) // []
Pseudocode:
  create a dictionary of opposite directions
  iterate over input array using reduce
    check to see if current element is equal to the previous elements opposite value in the dictionary
      true, return cumulative array with last element removed
      false, return cumulative array with current element added
...
function dirReduc(arr){
  ...
  return arr.reduce((p, c) => c === opposite[p[p.length - 1]] ? p.slice(0, -1) : p.concat(c), []);
}