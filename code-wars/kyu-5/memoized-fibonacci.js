...
Parameters: a num
  (position/index within fib sequence)
Returns: a num
  (value found at input position/index of fib sequence)
Examples:
  fibonacci(0), 0
  fibonacci(1), 1
  fibonacci(5), 5
  ...
  fibonacci(20), 6765
Pseudocode:
  initialize an outer fn (memo)
    initialize a sequence var assigned to an array containing 0 and 1
    return inner fn (takes in a num value)
      if sequence at arg index is undefined
        add the last 2 values in sequence and push to sequence
        recursively call inner fn with same arg
      return sequence at arg index
  initialize var to contain main fn, assigned to call to outer fn (memo)
*/

const memoizedFib = () => {
  const seq = [0, 1];
  const fib = n => {
    if (seq[n] === undefined) seq.push(seq.at(-2) + seq.at(-1)), fib(n);
    return seq[n];
  };
  return fib;
}
...
const memoizedFib = () => {
  const seq = [0, 1];
  const fib = n => {
    if (seq[n] === undefined) seq[n] = fib(n - 2) + fib(n - 1);
    return seq[n];
  };
  return fib;
}
