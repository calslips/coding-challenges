/**
 * In democracy we have a lot of elections. For example, we have to vote
 * for a class representative in school, for a new parliament or a new government.
...
Your Task
Return the name of the winner. If there is no winner, return null (in Java
  and JavaScript), None (in Python) or nil (in Ruby).
Task Description
...
arbitrary string, e.g. "A", "B", or "XJDHD".
...
A name wins the election if it gets more than n/2 votes (n = number of all
  votes, i.e. n is equal to the size of the given list).
Examples
...
The given list is immutable, i.e. you cannot modify the list (otherwise
  this could lead to vote rigging).
Good luck and have fun.
 */

function getWinner(listOfBallots) {
  const voteDict = listOfBallots.reduce((o, c) => ({...o, [c]: (o[c] || 0) + 1}), {});
  const highest = Object.entries(voteDict).sort((a, b) => b[1] - a[1])[0];
  return highest[1] > listOfBallots.length / 2 ? highest[0] : null;
}