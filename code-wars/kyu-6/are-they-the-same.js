...
Returns: a boolean
  whether the nums in arr2 are the squares
  of nums in arr1, occurring at the same freq
Examples:
    let a1 = [121, 144, 19, 161, 19, 144, 19, 11];
    let a2 = [11*11, 121*121, 144*144, 19*19, 161*161, 19*19, 144*144, 19*19];
    comp(a1, a2), true
Pseudocode:
  if either inputs are null
    return false
  initialize a hash
  iterate over nums of arr1
    if current num exists in hash as a key
      increment value
    otherwise
      set key to equal num with a value of 1
  iterate over nums of arr2
    if sqrt of current num does not exist in hash as key or value is 0
      return false
    otherwise
      decrement value at sqrt of current num key
  return true
*/
...
function comp(array1, array2) {
  ...
  const hash = {};
  for (const num of array1) hash[num] = ++hash[num] || 1;
