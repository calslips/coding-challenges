 ...
 *A person's Life Path Number is calculated by adding each individual
 number in that person's date of birth, until it is reduced to a single
 digit number.
...
integer between 1 and 9 which represents the Life Path Number of the given
date of birth.

You do not need to check that the input is correct format, you can assume
that it will always be a valid date (as a string) with given format.
...
function lifePathNumber(dateOfBirth) {
  return (nDob = +dateOfBirth.split('-').join('')) > 9 ? lifePathNumber(dateOfBirth.split('-').map(e => [...e].reduce((s, n) => s + +n, 0)).join('')) : nDob;
}